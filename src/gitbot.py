...
from internals.adapters.github import GitHubAdapter, GitHubListReposModel, GitHubFileModel, GitHubGroup
...
COMMENTS = open("comments.txt").read().replace("{TEMPLATE_REPO}", f"https://github.com/{ORG_NAME}/{TEMPLATE_REPO}")
...
if __name__ == "__main__":
    ...
    for repo in repo_list:
        ...
        if status == 200 and content.startswith(f"https://github.com/{ORG_NAME}/{TEMPLATE_REPO}"):
            ...
            branches = GitHubAdapter.get_branches(ORG_NAME, THIS_REPO, AUTH)  # type:ignore
            ...
            if created_branch:
                ...
                subprocess.run(F'git config --global user.email "xb500@users.noreply.github.com"', shell=True, cwd=branch_path, stdout=subprocess.DEVNULL)
                ...
                subprocess.run(F"git remote set-url --push origin {authenticated_url}", shell=True, cwd=branch_path, stdout=subprocess.DEVNULL)
                subprocess.run('git commit -m "sync with template"', shell=True, cwd=branch_path, stdout=subprocess.DEVNULL)
